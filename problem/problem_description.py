...
from utils.utils import generate_deterministic_problem, generate_problem_distrib, generate_problem_durations
...
class ProblemDescription:
    def __init__(
        ...
        if fixed and deterministic:
            if affectations is not None and durations is not None:
                if (
                    ...
                    raise Exception("Problem is specified to be deterministic, but you provided stochastic durations")
                self.affectations = affectations
                self.durations = durations
            else:
                if max_duration is None:
                    raise Exception("Please provide max_duration to generate a deterministic problem")
        ...
        # Second case: fixed and stochastic
            ...
            if affectations is not None and durations is not None:
                if (
                    ...
                    raise Exception("Problem is specified to be stochastic, but you provided deterministic durations")
                self.affectations = affectations
                self.durations = durations
            else:
        ...
        # Third case
            ...
            if durations is not None or affectations is not None:
                raise Exception("Please don't provide affectations or durations for unfixed problem")
            if max_duration is None:
                raise Exception("Please provide a valid max_duration for unfixed deterministic problem")
    ...
    def check_consistency(self, affectations, durations, n_jobs, n_machines):
        ...
        if (affectations is not None and durations is None) or (durations is not None and affectations is None):
            raise Exception("affectations xor durations is None. Please provide affectations and durations or none of them")
    ...
    def sample_problem(self):
        ...
        if self.fixed:
            ...
            if self.deterministic:
                return generate_deterministic_problem(self.n_jobs, self.n_machines, self.max_duration)
            else:
                affectations, durations = generate_problem_distrib(
                    self.n_jobs, self.n_machines, self.duration_mode_bounds, self.duration_delta
                )
