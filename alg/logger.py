...
def configure_logger(
    ...
    """
    if folder is None:
        folder = os.getenv("WHEATLEY_LOGDIR")
    if folder is None:
        folder = os.path.join(
            tempfile.gettempdir(),
            datetime.datetime.now().strftime("WHEATLEY-%Y-%m-%d-%H-%M-%S-%f"),
        )
    ...
    if format_strings is None:
        format_strings = os.getenv("WHEATLEY_LOG_FORMAT", "stdout,log,csv").split(",")
