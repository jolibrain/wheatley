pipeline {
  ...
  stages {
    ...
    stage('Performance Tests') {
      when {
       expression {!env.CHANGE_ID || pullRequest.labels.findAll { it == "ci:tests-perf" }.size() != 0 }
      }
      steps {
        lock(resource: null, label: "${NODE_NAME}-gpu", variable: 'LOCKED_GPU', quantity: 1) {
          sh 'printenv | sort'
          ...
          export CUDA_VISIBLE_DEVICES=$(echo ${LOCKED_GPU} | sed -n -e "s/[^,]* GPU \\([^[0-9,]]\\)*/\\1/gp")
          echo "Running on GPU ${CUDA_VISIBLE_DEVICES}"
          TORCH_HOME=/home/jenkins/app/.cache/ bash ./scripts/run_tests_perf.sh /home/jenkins/app/checkpoints/'''
        }
      }
    }
  }
