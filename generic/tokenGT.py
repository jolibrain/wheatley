...
from generic.flashEncoder import FlashAttentionTransformerEncoder
...
class TokenGT(nn.Module):
    ...
    def __init__(
        ...
        self.encoder = FlashAttentionTransformerEncoder(
            dim_model=hidden_dim, num_heads=num_heads, num_layers=num_layers
        )

        # self.fc_out = nn.Linear(hidden_dim, num_classes)
        # self.dropout = nn.Dropout(dropout)

    def forward_orig(self, data: Batch) -> torch.Tensor:
    ...
    def forward(self, g, features, edge_features, pe):
        ...
        d.x = features
        d.edge_attr = edge_features
        tokens, attention_masks = self.tokenizer(d)
        output = self.encoder(tokens, src_key_padding_mask=~attention_masks)
        return output.reshape((-1, self.hidden_dim))
