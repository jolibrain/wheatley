...
class AgentSpecification:
    def __init__(
        ...
        elif mlp_act.lower() == "swiglu":
            self.activation_fn = SwiGLU
        elif mlp_act.lower() == "geglu":
        ...
        elif mlp_act_graph.lower() == "swiglu":
            self.activation_fn_graph = SwiGLU
        elif mlp_act_graph.lower() == "geglu":
            self.activation_fn_graph = GeGLU
        else:
            raise Exception("Activation not recognized")
    ...
    def print_self(self):
        ...
        print(
            ...
            f"Graph convolution activation:     {self.gconv_activation.upper()}\n"
        ...
        first_features_extractor_shape = (
            f"FEATS -> {self.hidden_dim_features_extractor}"
        )
        ...
        actor_shape = (
            f""
            + "".join(
                [f" -> {self.hidden_dim_actor}" for _ in range(self.n_mlp_layers_actor)]
            )
            + " -> 1"
        )
        critic_shape = (
