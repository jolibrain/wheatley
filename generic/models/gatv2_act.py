...
class GATv2ActConv(MessagePassing):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # https://arxiv.org/abs/2501.13456
        self.kan = kan

        if activation == SwiGLU or activation == GeGLU or activation == GLU:
    ...
    def edge_update(
        ...
        if self.kan:
            x_concat = torch.concat([x_i, x_j], dim=-1)
            alpha = self.att_kan(x_concat).squeeze(-1)
            if edge_attr is not None and self.lin_edge is not None:
                if edge_attr.dim() == 1:
                    edge_attr = edge_attr.view(-1, 1)
                edge_attr = self.lin_edge(edge_attr)
                edge_attr = edge_attr.view(-1, self.heads, self.out_channels)
                alpha_edge = self.att_edge_kan(edge_attr).squeeze(-1)
