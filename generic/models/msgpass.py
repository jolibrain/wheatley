...
class MessagePassing(torch.nn.Module):
    ...
    def _check_input(
        ...
        if is_sparse(edge_index):
            ...
            if edge_index.dtype not in int_dtypes:
                raise ValueError(
                    f"Expected 'edge_index' to be of integer "
            ...
            if not torch.jit.is_tracing() and edge_index.size(0) != 2:
                raise ValueError(
                    f"Expected 'edge_index' to have size '2' in "
    ...
    def _lift(
        ...
        if not torch.jit.is_scripting() and is_torch_sparse_tensor(edge_index):
            ...
            if edge_index.layout == torch.sparse_coo:
                ...
                raise ValueError(
                    f"Unsupported sparse tensor layout (got '{edge_index.layout}')"
                )
            return src.index_select(self.node_dim, index)
        ...
        **kwargs: Any,
        ...
        if fuse:
            coll_dict = self._collect(
                self._fused_user_args, edge_index, mutable_size, kwargs
            )
            ...
            for i in range(decomposed_layers):
                ...
                coll_dict = self._collect(
                    self._user_args, edge_index, mutable_size, kwargs
                )
    ...
    def aggregate(
        ...
        return self.aggr_module(
            inputs, index, ptr=ptr, dim_size=dim_size, dim=self.node_dim
        )
    ...
    def edge_updater(
        ...
        **kwargs: Any,
        ...
        coll_dict = self._collect(
            self._edge_user_args, edge_index, mutable_size, kwargs
        )
    ...
    def decomposed_layers(self, decomposed_layers: int) -> None:
        if torch.jit.is_scripting():
            raise ValueError(
                "Inference decomposition of message passing "
    ...
    def explain(self, explain: Optional[bool]) -> None:
        if torch.jit.is_scripting():
            raise ValueError(
                "Explainability of message passing modules "
    ...
    def explain_message(
        ...
        if edge_mask is None:
            raise ValueError(
                "Could not find a pre-defined 'edge_mask' "
    ...
    def _set_jittable_templates(self, raise_on_error: bool = False) -> None:
        ...
        if not self.propagate.__module__.startswith(jinja_prefix):
            try:
                if (
                    "propagate" in self.__class__.__dict__
                    and self.__class__.__dict__["propagate"] != MessagePassing.propagate
                ):
        ...
        if self.inspector.implements(
            "edge_update"
        ) and not self.edge_updater.__module__.startswith(jinja_prefix):
            try:
                if (
                    "edge_updater" in self.__class__.__dict__
