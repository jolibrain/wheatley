...
class EEGATConv(nn.Module):
    ...
    def __init__(
        ...
        self._edge_scoring = edge_scoring
        if isinstance(in_node_feats, tuple):
            ...
            if self._edge_scoring:
                self.fc_edges_src = nn.Linear(
        ...
        if self._edge_scoring:
            self.fc_fij_e = nn.Linear(
                in_edge_feats, out_edge_feats * num_heads, bias=False
            )
            self.attn_e = nn.Parameter(
                torch.FloatTensor(size=(1, num_heads, out_edge_feats))
            )
        if bias:
    ...
    def reset_parameters(self):
        ...
        if self._edge_scoring:
            init.xavier_normal_(self.fc_edge_src.weight, gain=gain)
            init.xavier_normal_(self.fc_fij_e.weight, gain=gain)
            init.xavier_normal_(self.attn_e, gain=gain)

        init.constant_(self.bias, 0)

    def apply_edges(self, edges):
        f_fij_e = edges.data["f_fij_e"]
        a_e = edges.data["a_e"]
        e_out = f_fij_e * a_e
        return {"e_out": e_out}
    ...
    def forward(
        ...
        with graph.local_scope():
            ...
            if self._edge_scoring:
                f_fij_e = self.fc_fij_e(efeats_e)
            ...
            if self._edge_scoring:
                e_e = (f_out * self.attn_e).sum(dim=-1).unsqueeze(-1)
            ...
            if self._edge_scoring:
                graph.edata["f_fij_e"] = self.fc_edge_src(efeats_e).view(
                    -1, self._num_heads, self._out_edge_feats
                )
                graph.apply_edges(self.apply_edges)
                e_out = graph.edata.pop("e_out")
            ...
            if get_attention:
                if self._edge_scoring:
                    return h_out, f_out, e_out, graph.edata.pop("a")
                else:
                    return h_out, f_out, graph.edata.pop("a")
            else:
                if self._edge_scoring:
                    return h_out, f_out, e_out
                else:
                    return h_out, f_out
...
if __name__ == "__main__":
    ...
    eegat = EEGATConv(10, 10, 10, 10, 4, edge_scoring=True)
    eegat(
        edge_pred_graph,
        edge_pred_graph.ndata["feature"],
        edge_pred_graph.edata["feature"],
        edge_pred_graph.edata["feature"],
    )
