...
class Rcpsp:
    def __init__(self, n_jobs, n_modes_per_job, successors, durations, resource_cons, resource_availabilities, n_renewable_resources, n_nonrenewable_resources=0, n_doubly_constrained_resources=0, use_index_from_zero=False):
    ...
    def sample_topological(self, min_order, max_order):
        ...
        s_successors = []
        for j in s_jobs:
            s_successors.append([s_job_to_id[succ] for succ in self.successors_id[j] 
                if succ in s_jobs])               
        ...
        return Rcpsp(s_n_jobs, s_n_modes_per_job, s_successors, s_durations, 
            s_resource_cons, self.resource_availabilities, self.n_renewable_resources, 
