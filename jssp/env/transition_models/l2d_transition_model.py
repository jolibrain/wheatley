...
from generic.transition_model import TransitionModel
from jssp.utils.utils import job_and_task_to_node, node_to_job_and_task
            ...
            if job_availability_time + job_duration <= machine_occupancy[0][0]:
                ...
                for i in range(len(machine_occupancy) - 1):
                    ...
                    if start_time + duration < next_start_time:
                        if job_duration <= next_start_time - max(
                ...
                if index == -1:
                    ...
                    state.set_precedency(
                        machine_occupancy[-1][2], node_id, do_update=False
                    )
                    state.update_completion_times(node_id)
                    state.affect_node(node_id)
                else:
                    # The job is inserted between task_index and task_index+1
                    state.remove_precedency(
                        machine_occupancy[index][2], machine_occupancy[index + 1][2]
                    )
                    state.set_precedency(
                        machine_occupancy[index][2], node_id, do_update=False
                    )
                    state.set_precedency(
                        node_id, machine_occupancy[index + 1][2], do_update=False
                    )
                    state.update_completion_times(node_id)
                    state.affect_node(node_id)
